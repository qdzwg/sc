...
router.get("/seats",function (req,res) {
    ...
    async.waterfall([function (callback) {
        common.needleGet(areaUrl, req, res, function (err,result) {
            ...
            if(result.datas.isRefund){
                ...
                if(flag){
                    var seatData=[];
                    for (var m=0,mlen=showOrderList.length;m<mlen;m++){
                        var seatitem={
                            syncUrl:showOrderList[m].syncUrl,
                            theaterId:showOrderList[m].theaterId,
                            syncParame:{
                                "PPerformDate":moment(showOrderList[m].travelDate).format('YYYY-MM-DD'),
                                "PSeatAreaCode":showOrderList[m].areaCode,
                                "PPerformtime":showOrderList[m].showSessionTime
                            }
                        };
                        seatData.push(seatitem);
                    }
                    callback(null,seatData);
    ...
    },function (seatData,callback) {
        var funArray=[],fileArray=[];
        for(var i=0,len=seatData.length;i<len;i++) {
            var fileUrl = path.resolve('../') + "/public/data/seat/t" + seatData[i].theaterId + "s" + seatData[i].syncParame.PSeatAreaCode + ".json";
            if (!fs.existsSync(fileUrl)) {
                switch (i){
                    case 0:
                        funArray.push(function (callback) {
                            var infoUrl = seatData[0].syncUrl, seatParame = seatData[0].syncParame;
                            seatParame.METHOD = "GetSeatAreaDetail";
                            common.needlePost(infoUrl, seatParame, req, res, callback, "ajax");
                        });
                        break;
                    case 1:
                        funArray.push(function (callback) {
                            var infoUrl = seatData[1].syncUrl, seatParame = seatData[1].syncParame;
                            seatParame.METHOD = "GetSeatAreaDetail";
                            common.needlePost(infoUrl, seatParame, req, res, callback, "ajax");
                        });
                        break;
                    case 2:
                        funArray.push(function (callback) {
                            var infoUrl = seatData[2].syncUrl, seatParame = seatData[2].syncParame;
                            seatParame.METHOD = "GetSeatAreaDetail";
                            common.needlePost(infoUrl, seatParame, req, res, callback, "ajax");
                        });
                        break;
        ...
        if(funArray.length>0){
            async.parallel(funArray,function (err,resules) {
               for(var i=0,len=resules.length;i<len;i++){
                   if(resules[i].DATA.length>0){
                       fs.writeFileSync(fileArray[i],JSON.stringify(resules[i].DATA));
                       callback(null,seatData);
                   }else{
                       res.redirect('/seats/seatWarning?flag=1');
                   }
               }
            });
        }else{
        ...
        for(var i=0,len=seatData.length;i<len;i++){
            ...
            // funArray[i]=function (callback) {
            //         var infoUrl=seatData[i].syncUrl,seatParame=seatData[i].syncParame;
            //         seatParame.METHOD="GetSeatAreaMain";
            //         common.needlePost(infoUrl, seatParame, req, res, callback, "ajax");
            //     };
        }
        ...
        var options = { title: '选座' ,orderInfo:orderInfo,areaInfo:result,error: req.flash('error').toString(),mold:"seat"};
...
router.get('/seats/seatBooking',function (req,res) {
    var query=req.query,infoUrl=req.session.seatData[query.index].syncUrl;
    var seatData={
...
router.get('/getcoupon',function (req,res) {
    ...
    common.needleGet(url,req,res,function (err,result) {
        var options={title: '领取优惠券',mold:"coupon",item:result.datas,error: req.flash('error').toString()};
...
router.get('/seatArea',function(req,res){
    var options={title: '剧院区域',mold:"seat"};
