var swiper_one = new Swiper('.swiper-banner', {
...
$(function(){
    $(".index-tab").find("a").click(function(){
        $(this).parent().find("a").removeClass("active");
        $(this).addClass("active");
        var index=$(this).index();
        $(this).parent().next(".index-tab-content").find(".tab-panel").removeClass("active");
        $(this).parent().next(".index-tab-content").find(".tab-panel:eq("+index+")").addClass("active");
    });
...
for(n=1;n<9;n++){
    ...
    var mslide='slider'+n;
    var as=$("#" + page + "").find("a");
    $("#"+mslide).height($(".li_list").eq(0).height());
    var tt=new TouchSlider({id:mslide,'auto':'-1',fx:'ease-out',direction:'left',speed:600,timeout:5000,'before':function(index){
        $(".swipe").height($(".li_list").eq(index).height());
