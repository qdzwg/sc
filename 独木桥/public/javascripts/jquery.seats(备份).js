   ...
   var private_methods = {
        ...
        init:function(obj,opts){
		   var _this=this;
            ...
            var rowCounts=$("#rowCounts").val(),colCounts=$("#colCounts").val(),totalCount=$("#totalCount").val(),
                funArray=[function (callback) {
                    _this.ajaxSeat(opts.url[0],opts.param[0],callback);
                },function (callback) {
                    _this.ajaxSeat(opts.url[1],opts.param[1],callback);
                }];
            async.parallel(funArray,function (err,result) {
                var seatDetails=result[0],noSeatDetails=result[1].datas;
                for(var i=0,len=seatDetails.length;i<len;i++){
                    for(var j=0,nolen=noSeatDetails.length;j<nolen;j++){
                        if(seatDetails[i].SEATID==noSeatDetails[j].SEATID){
                            seatDetails.splice(i,1,noSeatDetails[j]);
                        }
                    }
                }
                var seatInfo=seatDetails;
                $("#seatMark").remove();
                var seats={
                    datas:{
                        DATA:seatInfo
                ...
                if(seats.datas.DATA.length>0){
                    $("#seatMove").find("li").each(function () {
                        if($(this).find("span").length==0){
                            var row=$(this).data("row"),col=$(this).data("col");
                            $(this).append(_this.seatSvg(row,col,seats.datas));
                        }
                    });
                    $(".seat-table").css({
                        top:-$("#seatMove").find("span:first").offset().top+$("#header").outerHeight(true)+$("#ticket-panel").outerHeight(true)+70,
                        left:-$("#seatMove").find("span:first").offset().left
                    });
                }
            });
        },
       ajaxSeat:function (url,param,callback) {
           var _this=this;
           param.areaCode=$("#areaCode").val();
           param.index=$("#select-ticket-type").find("option:selected").index();
           $.ajax({
               url:url,
               data:param,
               type:"get",
        ...
        seatSvg:function(row,col,data){
            var svg="",_this=this,seatdata=data.DATA;
