;(function($){
    ...
    var methods = {
        ...
        init : function (_options) {
            return this.each(function () {
                ...
                $this.on("click","td:not(.other)",function(){
                    var flagM=$(this).attr("data");
                    var day=$(this).data("day")=="今天"?mydate.getDate():$(this).data("day");
   ...
   var private_methods = {
       ...
       price:function(date,opts){
         var len=opts.settingdata.length,pricetext="",show=false,leftnum="";
           if(len>0){
               for(var i= 0;i<len;i++){
                   var targetdate=opts.settingdata[i].date;
                   var price=typeof opts.settingdata[i].price=="undefined"?"":opts.settingdata[i].price;
                   if(date==targetdate){
                       pricetext="<span class='price' data-ticket='"+opts.settingdata[i].ticket+"'><em>￥</em><strong>"+price+"</strong></span>";
                       leftnum=opts.settingdata[i].ticket;
                       show=true;
                   }
               }
           }
           return [pricetext,show,leftnum];
       },
        ...
        dateHtml: function(year, month,opts) {
            ...
            for (var j = 0; j < days; j++) {
                ...
                if(today == itemDay){
                    ...
                    html += " class='selecteds' data-day='"+(j + 1)+"'><span class='date-title'>余" + pricedata[2] + "</span><p class='calendar-explian'>"+pricedata[0]+"</p>";
                }else{
                    if(opts.optionsdays!=null||opts.settingdata.length>0){
