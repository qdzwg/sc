...
function filterUrl(query){
    ...
    if(url.indexOf('?') >= 0){
        ...
        queryArr.forEach(function (item,tags){
        	originalUrl.forEach(function (val,ind){
        		if (item.indexOf('orderStatus') >= 0 && val.indexOf('checkStatus') >= 0){
        			originalUrl.splice(ind,1);
        		}else if (item.indexOf('checkStatus') >= 0 && val.indexOf('orderStatus') >= 0){
        			originalUrl.splice(ind,1);
        		}
        	});
...
$(function (){
	...
	$('#dayTask').on('click', function (){
		getDayTask();
	});
	$('.task-box .box-title img').on('click', function (){
		$('.box-bac').hide();
		$('.task-box').hide();
	});
});
...
function getDayTask(){
	$.get('/user/task')
		.success(function (data){
			var _a = data.datas.memberTask,
				_b,
				_c;
			
			for (var i = 0; i < _a.length; i += 1){

				switch (_a[i].taskType){
					case '0': _b = '#'; break;
					case '1': _b = '#'; break;
					case '2': _b = '#'; break;
				}
				_c += '<tr>' +
						'<td>' + _a[i].taskName + '</td>' +
						'<td>' + _a[i].finishNum + '/' + _a[i].dayNum + '</td>' +
						'<td>' + _a[i].singleScore + '</td>' +
						'<td><a href="' + _b + '">去完成</a></td>' +
					  '</tr>'
			}
			$('.task-box .box-conetnt table tbody').html(_c);
			$('.box-bac').show();
			$('.task-box').show();
		})
		.error(function (err){
